...
class TransferThread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            #——————————————————读取ModBus数据————————————————————————————————————
            discrete_input_list = self.MM.read("transfer_modbus",address=0,count=4,function_code=cst.READ_DISCRETE_INPUTS)
            input_register_list = self.MM.read("transfer_modbus",address=2,count=3,function_code=cst.READ_INPUT_REGISTERS)
            ...
            lot = hex_to_string(self.MM.read("transfer_modbus",address=16,count=10,function_code=cst.READ_INPUT_REGISTERS))
            sn =hex_to_string(self.MM.read("transfer_modbus",address=26,count=10,function_code=cst.READ_INPUT_REGISTERS))
            ...
            if (trigger_camera and not trigger_camera_last) or (trigger_finished and not trigger_finished_last):
                ...
                template_pos_list = self.template_detector.detect(template,image_blur,self.template_detect_params)

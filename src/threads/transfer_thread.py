            ...
            if (trigger_camera and not trigger_camera_last) or (trigger_finished and not trigger_finished_last):
                ...
                if alarm_code != self._last_alarm_code:
                    try:
                        ...
                        ok, err = self.MM.write(alias="transfer_modbus", address=3, value_list=[coil_val], function_code=cst.WRITE_SINGLE_COIL)
                        if ok:
                ...
                if  trigger_finished == 1 and trigger_finished_last == 0:
                    ...
                    if final_alarm == 0 and self._last_alarm_code != 0:
                        try:
                            self.MM.write(alias="transfer_modbus", address=3, value_list=[0], function_code=cst.WRITE_SINGLE_COIL)
                            self._last_alarm_code = 0
                        except Exception as e:

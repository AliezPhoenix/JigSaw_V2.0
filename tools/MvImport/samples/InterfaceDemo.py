...
if __name__ == "__main__":
    ...
    try:
        ...
        ret =cam.MV_CC_SetEnumValue("StreamTriggerSource", 80)
        if ret != 0:
            raise Exception("set StreamTriggerSource fail! ret[0x%x]" % ret)
        else:
            print("set StreamTriggerSource [%d] success" % 80)
        
        ret = cam.MV_CC_SetCommandValue("StreamSoftwareTrigger")
        if ret != 0:
            raise Exception("set StreamSoftwareTrigger fail! ret[0x%x]" % ret)
        else:
            print("set StreamSoftwareTrigger success")

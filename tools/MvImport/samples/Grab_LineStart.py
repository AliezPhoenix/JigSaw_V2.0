...
def frame_to_opencv_image(frame_out_info, src_buf):
    ...
    if pixel_type in (
        ...
        return cv2.cvtColor(bayer_img, bayer_code_map[pixel_type])
...
def work_thread(cam=0, pData=0, nDataSize=0):
    ...
    while True:
        ...
        if out_frame_info.pBufAddr is not None and 0 == ret:
            ...
            if out_frame_info.stFrameInfo.nHeight > 1000:
                image = frame_to_opencv_image(out_frame_info, out_frame_info.pBufAddr)
                cv2.namedWindow("LineStart Grab", cv2.WINDOW_NORMAL)
                cv2.resizeWindow("LineStart Grab", 2000, 1000)
                cv2.imshow("LineStart Grab", image)
                cv2.waitKey() 
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        ret = cam.MV_CC_GetEnumValue("UserSetSelector", st_enum_value)
        print("user_set_selector: " , st_enum_value.nCurValue)
 
        ret = cam.MV_CC_SetEnumValue("UserSetSelector", 1)
        if ret != 0:
            raise Exception("error: set user set selector fail! ret[0x%x]" % ret)

        ret = cam.MV_CC_SetCommandValue("UserSetLoad")
        if ret != 0:
            raise Exception("error: load user set fail! ret[0x%x]" % ret)

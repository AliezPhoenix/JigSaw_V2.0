...
class DryPramasSetDialog(Ui_DryPramasSetDialog, QDialog):
    def __init__(self,config_manager:'ConfigManager',parent=None):
        ...
        # Mark检测区域相关按钮
        self.pushButton_create_mark_roi_dry.clicked.connect(lambda: self.create_roi("mark"))
        self.pushButton__clear_mark_roi_dry.clicked.connect(lambda:self.clear_check_roi("mark"))
        ...
        self.pushButton_remove_mark_roi_dry = QPushButton("删除选中")
        self.pushButton_remove_mark_roi_dry.clicked.connect(self._remove_selected_mark_roi_dry)
        hl.addWidget(self._mark_roi_list)
        hl.addWidget(self.pushButton_remove_mark_roi_dry)
    ...
    def _remove_selected_mark_roi_dry(self):
    ...
    def update_params(self):
        ...
        try:
            ...
            if not self.is_init:
                ...
                self.lineEdit_min_mak_area_dry.setText(str(min_mark_area))
    ...
    def _update_params_from_ui(self):
        ...
        try:
            ...
            if hasattr(self, 'lineEdit_min_mak_area_dry') and self.lineEdit_min_mak_area_dry.text().strip():
                self.local_params["min_mark_area"] = int(float(self.lineEdit_min_mak_area_dry.text()))
    ...
    def confirm_load(self):
        ...
        try:
            ...
            other = self.config_manager.get_section("work_transfer_params")
            other["mark_rois"] = list(self.local_params.get("mark_rois", []))
            other.pop("mark_roi", None)
            self.config_manager.set_section("work_transfer_params", other)
